...
You can customize `docums-print-site-plugin` in your `docums.yml` with the following settings:
...
`enumerate_headings`
:   Default `true`. This will add numbering (enumeration) to all headings and sections, as well as the table of contents. Note this will only enumerate the print site page; if you want to enumerate the entire site, you can use [docums-enumerate-headings-plugin](https://github.com/khanhduy1407/docums-enumerate-headings-plugin).
...
`enumerate_figures`
:   Default `true`. This will add numbering to all figure captions (for example "Figure 1: <caption>"). Works especially well with [docums-img2fig-plugin](https://github.com/khanhduy1407/docums-img2fig-plugin).

`add_cover_page`
:   Default `false`. When enabled, a cover page is added to the print page, displaying the `site_title` and other information from the `docums.yml` file. See also [Customizing the cover page](how-to/cover_page.md)
...
`include_css`
: Default is `true`. When disabled the [print-site stylesheets](https://github.com/khanhduy1407/docums-print-site-plugin/tree/master/docums_print_site_plugin/css) are not included. This makes it easy to overwrite the CSS with your own stylesheets, using the [extra_css](https://khanhduy1407.github.io/docums/user-guide/configuration/#extra_css) option in your `docums.yml` file.
